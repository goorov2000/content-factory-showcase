...
# Правка pattern-analyzer: визуальные факты как источник верификации

## Проблема

Анализатор слеп: раздел «Визуал» его паттернов — догадка по капшену и
транскрипту, а не наблюдение (слепота зафиксирована в Problem Statement
спеки визуального контура). С тикета 07 winners/losers несут ему поле
`visual_facts` — структурированные факты vision-звена, — но действующий
промпт про это поле не знает: верификация evidence разрешает только
транскрипт и капшен, а пустое `visual_facts` ничем не отличается от
«визуала нет».

## Что предлагается

Три правки промпта (полный текст ниже):

1. «Вход» упоминает `visual_facts` в winners/losers analysis-JSON.
2. Правило верификации evidence расширено: «подтверждено transcript_text,
   однозначным капшеном ИЛИ визуальными фактами строки».
3. Новый раздел «Визуальные факты (visual_facts)»: пустое поле = «медиа не
   разобрано», а не «фактов нет» — выдумывать визуал запрещено; фактами
   считаются только `visual_evidence`, гипотезы (`visual_hypotheses`) в
   evidence не идут; паттерн с визуальным основанием кладёт машинную ссылку
   в `evidence.visual_refs` (форма из schemas/patterns.schema.json, тикет 08).

## Evidence (правило №1)

- Спека: `docs/plans/2026-08-14-visual-contour/spec.md` — §«Доставка в
  анализ и контракт честности вниз»; Problem Statement (слепота анализатора:
  «Визуал» — догадка по тексту).
- Тикет: `docs/plans/2026-08-14-visual-contour/issues/09-prompt-proposals-analysis-layer.md`.
- Код: `src/cf/analyze.py` — `visual_facts` в ROW_FIELDS (тикет 07);
  `schemas/patterns.schema.json` — опциональный `evidence.visual_refs`
  (тикет 08); `schemas/visual-facts.schema.json` — структура фактов.

## Применение

```bash
cd /home/<user>/projects/CF
.venv/bin/python -m cf apply-agent-edit 2026-08-14-pattern-analyzer-visual-facts.md
```

## Файл 1: prompts/agents/pattern-analyzer.md
